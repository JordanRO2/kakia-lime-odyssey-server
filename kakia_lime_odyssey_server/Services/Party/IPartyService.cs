	...
	public PARTY_MEMBER ToPacketStruct()
	{
		...
		return new PARTY_MEMBER
		{
			...
			state = new PARTY_MEMBER_STATE
			{
				...
				hp = status?.BasicStatus.Hp ?? 0,
				mhp = status?.BasicStatus.MaxHp ?? 0,
				mp = status?.BasicStatus.Mp ?? 0,
				mmp = status?.BasicStatus.MaxMp ?? 0,
				lp = status?.BasicStatus.Lp ?? 0,
				mlp = status?.BasicStatus.MaxLp ?? 0,
